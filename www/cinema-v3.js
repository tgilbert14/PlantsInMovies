...
(() => {
  ...
  document.addEventListener('click',e=>{
    ...
    if(e.target.closest('.world-button')){
      ...
      if(root.dataset.motion!=='off')requestAnimationFrame(()=>root.classList.add('reel-changing'));
    }
  ...
  function ready(){
    ...
    if('IntersectionObserver' in window){
      const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){if(root.dataset.motion==='off')entry.target.dataset.arrivalDone='true';else entry.target.classList.add('reveal-ready');observer.unobserve(entry.target);}}),{threshold:.08});
    ...
    if(window.Shiny)Shiny.addCustomMessageHandler('notebook-saved',message=>{
      ...
      if(button){button.classList.remove('notebook-stamped');if(root.dataset.motion!=='off')requestAnimationFrame(()=>button.classList.add('notebook-stamped'));}
    });
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ready,{once:true});else ready();
})();

/* Completed arrivals stay settled across Pause/Resume. Rewards belong to one action. */
(() => {
  const root=document.documentElement;
  const arrivals='.atlas-panorama,.world-scene,.scene-specimen .botanical-art,.notebook-card,.reveal-ready';
  const finite=new Set(['atlas-open','scene-seat','plate-develop','page-arrive','note-press']);
  document.addEventListener('animationend',event=>{
    if(finite.has(event.animationName))event.target.dataset.arrivalDone='true';
    if(event.animationName==='stamp-press')event.target.classList.remove('notebook-stamped');
    if(event.animationName==='reel-seat')root.classList.remove('reel-changing');
  });
  function settle(){
    if(root.dataset.motion!=='off')return;
    document.querySelectorAll(arrivals).forEach(el=>{el.dataset.arrivalDone='true';});
    root.classList.remove('reel-changing');
    document.querySelectorAll('.notebook-stamped').forEach(el=>el.classList.remove('notebook-stamped'));
  }
  new MutationObserver(settle).observe(root,{attributes:true,attributeFilter:['data-motion'],childList:true,subtree:true});
  settle();
})();
